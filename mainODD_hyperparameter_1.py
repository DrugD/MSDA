...
sys.path.insert(0, "/home/lk/project/ODD/MSDA_DRP/models/hyperparam")
...
import datetime
from utils import *
from models.hyperparam.odd_deepcdr_remainRawFusion import ODDDeepCDR as ODDDeepCDR_RRF
from models.hyperparam.odd_graphdrp_remainRawFusion import ODDGraphDRP as ODDGraphDRP_RRF
from models.hyperparam.odd_deepttc_remainRawFusion import ODDDeepTTC as ODDDeepTTC_RRF
from models.hyperparam.odd_graphtransdrp_remainRawFusion import ODDGraTransDRP as ODDGraTransDRP_RRF
from models.hyperparam.odd_tcnns_remainRawFusion import ODDtCNNs as ODDtCNNs_RRF
from models.hyperparam.odd_transe_remainRawFusion import ODDTransE as ODDTransE_RRF
...
def train(model, device, loaders, optimizer, epoch, log_interval, args):
    ...
    data_target_iter = iter(loaders[1])
    ...
    for batch_idx in range(len_dataloader):
        ...
        sys.stdout.write('\r epoch: %d, [iter: %d / all %d],  %f %f'
                         % (epoch, batch_idx + 1, len_dataloader, reg_loss1.data.cpu().numpy(), reg_loss1.data.cpu().numpy()))
        sys.stdout.flush()

    # pdb.set_trace()
    return reg_loss1.data.cpu().numpy()
...
def predicting(model, device, loader, loader_type, args):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            output_raw, output1, output_mean = model(data)
...
def main(config, yaml_path):
    ...
    for item in tqdm(test_smiles_pt):
        ...
        for item_dataset in [top_s_data]:
