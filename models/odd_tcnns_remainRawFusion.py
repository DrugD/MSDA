...
class ODDtCNNs(torch.nn.Module):
    ...
    def forward(self,  source, target=None, mark=None):
        ...
        # print(self.cell_module.backbone[0].weight.sum().item())
        # print(self.cell_module.backbone[3].weight.sum().item())
        # print(self.cell_module.backbone[6].weight.sum().item())
        # print(self.cell_module.backbone[0].bias.sum().item())
        # print(self.cell_module.backbone[3].bias.sum().item())
        # print(self.cell_module.backbone[6].bias.sum().item())
        ...
        if self.training == True:
            ...
            target_x_fusion_1_feature, _ = self.fusion_module1(
                target_x_drug, target_x_cell)
            target_x_fusion_2_feature, _ = self.fusion_module2(
                target_x_drug, target_x_cell)

            # print(mark)
            # print( self.fusion_module.fc1.weight.sum() )
            # print( self.fusion_module.fc2.weight.sum() )
            # print( self.fusion_module.fc3.weight.sum() )
            # print("----------------------------------")
            # print(target_x_drug.sum(),target_x_cell.sum())
            # print(self.fusion_module(target_x_drug, target_x_cell)[1].sum())
            # print("----------------------------------")
            # pdb.set_trace()

            target_x_fusion_1_feature, re_index_t_1 = self.re_idx_feature_by_cell(
                target, target_x_fusion_1_feature)
            target_x_fusion_2_feature, re_index_t_2 = self.re_idx_feature_by_cell(
                target, target_x_fusion_2_feature)

            if mark == 1:
                source_x_fusion_feature, source_x_fusion_reg = self.fusion_module1(
                    source_x_drug, source_x_cell)

                source_x_fusion_feature, re_index_s_1 = self.re_idx_feature_by_cell(
                    source, source_x_fusion_feature)

                mmd_loss_, source_align, target_align = mmd_align(
                    source_x_fusion_feature, target_x_fusion_1_feature, re_index_s_1, re_index_t_1)

                mmd_loss += mmd_loss_ * 1

                reg_loss = self.loss_fn(source_x_fusion_reg.float(
                ), source.y.view(-1, 1).float().to(device))

                rank_loss = self.ranking_loss(source.y.view(-1, 1).float().to(
                    device), source_x_fusion_reg.float(), torch.ones_like(source_x_fusion_reg.float()))
            ...
            if mark == 2:
                source_x_fusion_feature, source_x_fusion_reg = self.fusion_module2(
                    source_x_drug, source_x_cell)
                ...
                mmd_loss_, source_align, target_align = mmd_align(
                    source_x_fusion_feature, target_x_fusion_2_feature, re_index_s_1, re_index_t_2)
            ...
            _, target_x_fusion_reg_0 = self.fusion_module(
                source_x_drug, source_x_cell)
            _, target_x_fusion_reg_1 = self.fusion_module1(
                source_x_drug, source_x_cell)
            _, target_x_fusion_reg_2 = self.fusion_module2(
                source_x_drug, source_x_cell)

            target_x_fusion_reg_0_reweight = np.dot(
                1, target_x_fusion_reg_0.cpu())
            target_x_fusion_reg_1_reweight = np.dot(
                1, target_x_fusion_reg_1.cpu())
            target_x_fusion_reg_2_reweight = np.dot(
                1, target_x_fusion_reg_2.cpu())

            target_x_fusion_reg_mean = torch.Tensor(np.array(
                [target_x_fusion_reg_0_reweight, target_x_fusion_reg_1_reweight, target_x_fusion_reg_2_reweight]).sum(0))/3

            return target_x_fusion_reg_0_reweight, target_x_fusion_reg_1_reweight, target_x_fusion_reg_2_reweight, target_x_fusion_reg_mean
