...
class Drug(nn.Module):
    def __init__(self,
        ...
        self.DeepTTC_encode = torch.load("/home/lk/project/MSDA_DRP/data/processed/DeepTTC_encode.pth")
...
class Fusion(nn.Module):
    ...
    def forward(self, drug, cell):
        ...
        if self.fusion_mode == "concat":
            x_feature = torch.cat((drug, cell), 1)
        
        x = self.backbone(x_feature)
        return x_feature, x


class ODDDeepTTC(torch.nn.Module):
    def __init__(self, config):
        super(ODDDeepTTC, self).__init__()
    ...
    def forward(self,  source, target=None, mark=None):
        ...
        if self.training == True:
            ...
            target_x_fusion_reg_mean = torch.Tensor(np.array([target_x_fusion_reg_0_reweight, target_x_fusion_reg_1_reweight, target_x_fusion_reg_2_reweight] ).sum(0))/3
