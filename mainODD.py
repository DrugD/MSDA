...
from models.odd_transe_remainRawFusion import ODDTransE as ODDTransE_RRF
...
def main(config, yaml_path):
    ...
    Similarity_matrix = np.load(
        ROOTDIR+"/data/Similarity_matrix_by_wasserstein_GDSCv2.npy", allow_pickle=True).item()
    ...
    for item in tqdm(test_smiles_pt):
        ...
        work_dir = ROOTDIR + "/exp/" + \
            config['marker'] + "/" + work_dir + "_" + item + date_info

        if not os.path.exists(ROOTDIR+"/exp/" + config['marker']):
            os.mkdir(ROOTDIR+"/exp/" + config['marker'])
