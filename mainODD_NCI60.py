...
sys.path.insert(0,"/home/lk/project/MSDA_DRP/models")
...
from models.odd_transe_remainRawFusion_NCI60 import ODDTransE as ODDTransE_RRF
from models.odd_tcnns_remainRawFusion import ODDtCNNs as ODDtCNNs_RRF
from models.odd_graphtransdrp_remainRawFusion import ODDGraTransDRP as ODDGraTransDRP_RRF
from models.odd_deepttc_remainRawFusion import ODDDeepTTC as ODDDeepTTC_RRF
from models.odd_graphdrp_remainRawFusion import ODDGraphDRP as ODDGraphDRP_RRF
from models.odd_deepcdr_remainRawFusion import ODDDeepCDR as ODDDeepCDR_RRF
...
def train(model, device, loaders, optimizer, epoch, log_interval, args):
    ...
    data_target_iter = iter(loaders[2])
    ...
    for batch_idx in range(len_dataloader):
        ...
        # print(gamma)
        loss1 = reg_loss1 * 0.2 +  mmd_loss * gamma
        loss1.backward()
        optimizer.step()

        reg_loss2, mmd_loss = model(data_source_2, data_target1, mark=2)
        # gamma = 2 / (1 + math.exp(-10 * (batch_idx) / (len_dataloader+1) )) - 1
        loss2 = reg_loss2 * 0.2 +  mmd_loss * gamma
        loss2.backward()
        optimizer.step()
        
        
        sys.stdout.write('\r epoch: %d, [iter: %d / all %d],  %f %f' \
              % (epoch, batch_idx + 1, len_dataloader, reg_loss1.data.cpu().numpy(), reg_loss2.data.cpu().numpy()))
        sys.stdout.flush()
    
    # pdb.set_trace()
    return np.mean(reg_loss1.data.cpu().numpy()+reg_loss2.data.cpu().numpy())
...
def predicting(model, device, loader, loader_type, args):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            output_raw, output1, output2, output_mean = model(data)

            total_pred_raw = torch.cat((total_pred_raw, torch.tensor(output_raw)), 0)
...
def main(config, yaml_path):
    ...
    # test_smiles_pt = [ 'Belinostat', 'XMD8-85', 'GSK319347A', 'Tamoxifen', 'QL-XII-47', 'IOX2', 'Piperlongumine', 'PF-562271', 'Y-39983', 'Alectinib', 'Ponatinib', 'TAK-715', 'JW-7-24-1', 'Vinblastine', 'GW-2580', 'Zibotentan', 'Sepantronium bromide', 'Cytarabine', '5-Fluorouracil', 'Navitoclax', 'Rucaparib', 'JNK-9L', 'Pelitinib']
    test_smiles = np.load("/home/lk/Data/NCI60_dataset/total_32310_process/NCI60_drug_smiles.npy")
    test_smiles_pt = np.load("/home/lk/Data/NCI60_dataset/total_32310_process/NCI60_drug_name.npy")
    ...
    Similarity_matrix = np.load("/home/lk/Data/NCI60_dataset/NCI60_Similarity/Similarity_matrix_by_wasserstein_NCI60_32310_30%.npy",allow_pickle=True).item()
    ...
    for item in tqdm(test_smiles_pt):
        ...
        pretrain_model = NCI60_30_pretrain_model_dict[config['model_name'].split('_')[0][3:]]
   
        model = load_weight(pretrain_model, model, "RRF" in config["model_name"])
        ...
        work_dir = "/home/lk/project/MSDA_DRP/exp/" + config['marker'] + "/" + work_dir + "_"  + item +  date_info
        ...
        # 
        Target_dataset = load_dataset_from_smiles(config["dataset_type"],config['dataset_type']['test'])
        
        test_aim_sorted =  Similarity_matrix[Target_dataset[0].smiles]
        test_aim_sorted = sorted(test_aim_sorted.items(),  key=lambda d: d[1], reverse=False)
        ...
        test_aim_sorted_top = test_aim_sorted_del[:Ktop]
 
        # test_aim_sorted_down = test_aim_sorted_del[-Ktop-1:-1]
        test_aim_sorted_down = test_aim_sorted_del[Ktop:Ktop*2]
        # print(test_aim_sorted_top, test_aim_sorted_down)
        
        source_domain_list_top = []
        
        source_domain_list_down = []
        
        for idx in range(len(test_aim_sorted_top)):
            source_domain_list_top.append(
                load_dataset_from_smiles(
                    config["dataset_type"], 
                    smiles2pt_mp[test_aim_sorted_top[idx][0]]
                    )
                )
    
            
        for idx in range(len(test_aim_sorted_down)):
            source_domain_list_down.append(
                load_dataset_from_smiles(
                    config["dataset_type"], 
                    smiles2pt_mp[test_aim_sorted_down[idx][0]]
                    )
                )
        
        
        top_s_data = ConcatDataset(source_domain_list_top)
   
        down_s_data = ConcatDataset(source_domain_list_down)
        ...
        loaders_train = []
        for item_dataset in [top_s_data, down_s_data]:
            loaders_train.append(DataLoader(item_dataset, batch_size=max_batchsize, shuffle=True, drop_last = True))
    
        loaders_test = DataLoader(Target_dataset, batch_size=max_batchsize, shuffle=False, drop_last = False)
        ...
        for idx, loader_item in enumerate(loaders_train):
            print("TrainLoader-{} on {} samples...".format(idx, len(loader_item.dataset)))
        ...
        for epoch in tqdm(range(num_epoch+1)):
            ...
            res_test_list = predicting(model, device, loaders_test, "test", config)
...
def load_weight(pretrain_model, model, remainRaw = False):
    ...
    pretrain_weight= torch.load(pretrain_model, map_location=torch.device('cpu'))
